...
if (metaElement) {
    ...
    if (metaElement.content === "my_profile_users" || metaElement.content === "new_places" || metaElement.content === "general_settings" ) {
        ...
        async function onDragEnd() {
            ...
            const URL = `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(`${lngLat.lng},${lngLat.lat}`)}.json?access_token=pk.eyJ1Ijoid2F5ZGRhIiwiYSI6ImNrZzYwZWJiYzB6bjMycW5udmd1NHNscDAifQ.wkmzM9Mh8XyPXZ8BgpyJXg&cachebuster=1596775236930&autocomplete=true&country=mx&bbox=-102.36584333677,18.203715736351,-95.646605055518,20.200815919313&proximity=-99.630833,19.354167&limit=1`;
            try {
                let response = await (
                    await fetch(URL, {
                        method: "GET"
